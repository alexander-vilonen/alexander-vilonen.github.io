//http://stegu.github.io/webgl-noise/webdemo/
...
float color(vec2 xy) { return 0.7 * snoise(vec3(xy, 0.25*u_time)); }

void main() {
    vec4 gradientColor = mix(u_top_color, u_bottom_color, v_texCoords.y);

    //Simplex noise stuff
    vec2 p = (gl_FragCoord.xy/u_resolution.y) * 2.1 - 1.0;  //I change the 2.1 to zoom in/out

    vec3 xyz = vec3(p, 0);
    float n = color(xyz.xy);

    gradientColor.xyz = gradientColor.xyz * vec3(0.97 + 0.4 * vec3(n, n, n));
